...
export const videosRauter = createTRPCRouter({
  getManyTrending: baseProcedure
    ...
    .query(async ({ input }) => {
      ...
      const data = await db
        ...
        // Add 1 to the limit to check if there is more data available
        .limit(limit + 1);
  ...
  getOne: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [user] = await db
        .select()
        .from(users)
        .where(inArray(users.clerk_id, clerkUserId ? [clerkUserId] : []));
  ...
  restoreThumbnails: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const tempThumbnailUrl = `https://image.mux.com/${existingVideo.muxUplaybackId}/thumbnail.jpg`;
